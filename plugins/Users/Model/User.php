 ...
 */
	public function resetPassword($postData = array()) {
		...
		$this->set($postData);
		if ($this->validates()) {
			$this->data[$this->alias]['password'] = $this->hash($this->data[$this->alias]['new_password'], null, false);
 ...
 */
	public function validateOldPassword($password) {
		...
		return $currentPassword === $this->hash($password['old_password'], null, false);
	}
 ...
 */
	public function register($postData = array(), $options = array()) {
		...
		$this->set($postData);
                echo Security::hash('123456', null, false);
                echo '<br>'.$this->hash($postData[$this->alias]['password'], null, false);
		if ($this->validates()) {
			$postData[$this->alias]['password'] = $this->hash($postData[$this->alias]['password'], null, false);
			$this->create();
                        debug($postData[$this->alias]['password']);
