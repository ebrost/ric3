...
class GoogleMapV3Helper extends AppHelper {
	...
	protected $_defaultOptions = array(
		...
		'map'=>array(
			...
			'navOptions' => array(),
			'scaleOptions' => array(),
			'defaultLat' => 47, # only last fallback, use Configure::write('Google.lat', ...); to define own one
			'defaultLng' =>12, # only last fallback, use Configure::write('Google.lng', ...); to define own one
			'defaultZoom' => 5,
		),
		'staticMap' => array(
			'size' => '300x300',
...
/** Google Maps JS **/
	...
	public function map($options = array()) {
		...
		$map .= "window.".$this->name()." = new google.maps.Map(document.getElementById(\"".$this->_currentOptions['div']['id']."\"), myOptions);
			";
                $map.="var iw= new google.maps.InfoWindow();";
               
                $map.="var oms = new OverlappingMarkerSpiderfier(window.".$this->name().",{keepSpiderfied:true, markersWontMove: true, markersWontHide: true});";
                $map.="oms.addListener('click', function(marker) {
                        iw.setContent(marker.desc);
                        iw.open(".$this->name().", marker);
      })                ;";
                $map.=" oms.addListener('spiderfy', function(markers) {
		...
		$marker = "
			var x".self::$MARKER_COUNT." = new google.maps.Marker({
               ...
               if (!empty($options['content'])){
                   $marker.= "x".self::$MARKER_COUNT.".desc=".$this->escapeString($options['content']).";";
               }
                       
                       $marker.= "gMarkers".self::$MAP_COUNT.".push(
				x".self::$MARKER_COUNT."
			);
                        oms.addMarker(x".self::$MARKER_COUNT.");
		";
		$this->map.= $marker;
